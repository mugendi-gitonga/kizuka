...
from pricing.models import PROVIDER_CHOICES, BusinessPricingPlan, CountryTax
...
class DepositRequest(AliasModel):
        ...
        try:
            if self.provider == "MPESA-C2B":
                ...
                status_code, response = processor.send_stk_push(
                    self.phone_number, self.amount, self.reference, self.narration
                )
    ...
    def complete(self):
        ...
        with db_transaction.atomic():
            trans = wallet.credit(self.amount, reference=self.reference, trans_type=f"DEP-{self.alias}", description=f"Deposit of {self.amount} for INV-{self.alias}")
